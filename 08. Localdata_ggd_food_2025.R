...
foodshop <- read.csv("ggd_food_2025.csv", na="", stringsAsFactors = F)
...
foodshop$district <- ifelse(foodshop$district%in%c("106호","번지","회","시 강남","시 계양","시 관악","시 금천","시 남동","시 노원","시 마포","시 미추","시 서구","시 용산","시 은평","자치도","회"),NA,foodshop$district)#이상치제거
table(foodshop$district)#이상치 확인
